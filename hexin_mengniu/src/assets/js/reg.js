...
//设置cookies 
function setCookie30(name, value) {
	var Days = 30;
	var exp = new Date();
...
function isCardIDs(sId) {
    ...
    if (sBirthday != (d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate())) {
        map["STATUS"] = "error";
        map["ERROR"] = "身份证上的出生日期非法";
        return map;
    }
    for (var i = 17; i >= 0; i--) iSum += (Math.pow(2, i) % 11) * parseInt(sId.charAt(17 - i), 11);
    if (iSum % 11 != 1) {
        map["STATUS"] = "error";
        map["ERROR"] = "你输入的身份证号非法";
        return map;
    }
    map["STATUS"] = "success";
    var dY = Number(sId.substr(10, 2));
    if (dY <= 9) {
        dY = "0" + dY;
    }
    var dR = Number(sId.substr(12, 2));
    if (dR <= 9) {
        dR = "0" + dR;
    }
    var sBirthdayData = sId.substr(6, 4) + "-" + dY + "-" + dR;
    map["ERROR"] = aCity[parseInt(sId.substr(0, 2))] + "," + sBirthdayData + "," + (sId.substr(16, 1) % 2 ? "男" : "女");
    return map;
}
